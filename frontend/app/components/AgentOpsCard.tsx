  ...
  return (
    <div className="bg-slate-800 border border-slate-700 rounded-xl p-5">
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <a
          href={metrics.session_replay_url ?? "https://app.agentops.ai"}
          target="_blank"
          rel="noopener noreferrer"
          className="text-violet-400 hover:text-violet-300 text-xs transition-colors flex items-center gap-1"
        >
          {metrics.session_replay_url ? "View Session Replay →" : "agentops.ai →"}
        </a>
      </div>

      {/* Session replay banner — only shown when URL is available */}
      {metrics.session_replay_url && (
        <a
          href={metrics.session_replay_url}
          target="_blank"
          rel="noopener noreferrer"
          className="flex items-center gap-2 px-3 py-2 mb-4 rounded-lg bg-violet-500/10 border border-violet-500/20 hover:bg-violet-500/15 transition-colors group"
        >
          <span className="relative flex h-2 w-2 flex-shrink-0">
            <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-violet-400 opacity-75" />
            <span className="relative inline-flex rounded-full h-2 w-2 bg-violet-400" />
          </span>
          <span className="text-violet-300 text-xs font-medium flex-1">Live AgentOps session — view all TinyFish runs</span>
          <Activity className="w-3 h-3 text-violet-400 group-hover:text-violet-300 transition-colors flex-shrink-0" />
        </a>
      )}
